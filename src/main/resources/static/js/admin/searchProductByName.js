...
document.addEventListener("DOMContentLoaded", function () {
    document.getElementById('productSearch').value = window.location.pathname.substring(19)
    getListProduct();
    categories()
        ...
        if (draggedProduct) {
            ...
            if (targetRow) {
                ...
                if (targetProduct) {
                    ...
                    if(draggedProduct.id == listProduct[listProduct.length-1].id){
                        listProductRequest.push({
                            productId: listProduct[listProduct.length-1].id,
                            orderSort: listProduct[listProduct.length-2].orderSort - 1
                        })
                    }
                    else {
                        listProductRequest.push({
...
function fetchlistProduct(index) {
    ...
    xhttp.open("GET", `${domain}/api/v1.0/ProductByName/${window.location.pathname.substring(19)}?pageNo=${index}&pageSize=${pageSize}`, false);
    xhttp.setRequestHeader("Content-type", "application/json");
    token = localStorage.getItem("Token");
    authorization = 'Bearer ' + token;
    xhttp.setRequestHeader("Authorization", authorization);
